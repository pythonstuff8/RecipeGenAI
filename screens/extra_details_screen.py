...
class ExtraDetailsScreen(MDScreen):
    ...
    @classmethod
    def display_recipe(cls, incoming_page):
        cls.incoming_page = incoming_page
    ...
    def setup_layout(self):
        ...
        subheader = MDLabel(
            text=(
                "Add your extra details, if there is something that you want "
                "to add but is not available then put it in the other notes section"
            ),
            halign="center",
            size_hint_y=None,
        ...
        for allergy in allergies:
            chip = CustomFilterChip(text=allergy)
        ...
        for pref in prefs:
            chip = CustomFilterChip(text=pref)
        ...
        slider = MDSlider(
            min=0, max=4, value=4, step=1, size_hint_y=None, height="48dp"
        )
        slider.bind(
            value=lambda instance, val: self.time_slider_label.setter("text")(
        ...
        for meal in meal_types:
            chip = CustomFilterChip(text=meal, on_release=self.handle_chip_selection)
        ...
        self.cusine_grid = MDGridLayout(
            cols=2, spacing=10, size_hint=(None, None), width=500, height="250dp"
        )
        cusine_types = [
            "American",
            "Italian",
        ...
        for cusine in cusine_types:
            chip = CustomFilterChip(text=cusine, on_release=self.handle_chip_selection2)
        ...
        for equip in equipment:
            chip = CustomFilterChip(text=equip, active=True)
        ...
        layout.add_widget(MDLabel(text="", size_hint_y=None, height="40dp", opacity=0))
        layout.add_widget(diet_label)
        layout.add_widget(self.diet_grid)
        layout.add_widget(MDLabel(text="", size_hint_y=None, height="25dp", opacity=0))
        ...
        layout.add_widget(
            MDLabel(text="", size_hint_y=None, height="200dp", opacity=0)
        )
    ...
    def go_home(self, instance):
        ...
        self.manager.current = f"{ExtraDetailsScreen.incoming_page}"
